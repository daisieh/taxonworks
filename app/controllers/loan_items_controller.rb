class LoanItemsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @loan_item.save
        format.html { redirect_back fallback_location: hub_url, notice: 'Loan item was successfully created.' }
        format.json { render json: @loan_item, status: :created, location: @loan_item }
      else
        format.html { redirect_back fallback_location: hub_url, notice: 'Loan item was NOT successfully created.' }
  ...
  def update
    respond_to do |format|
      if @loan_item.update(loan_item_params)
        format.html { redirect_back fallback_location: hub_url, notice: 'Loan item was successfully updated.' }
        format.json { render :show, status: :ok, location: @loan_item }
      else
        format.html { redirect_back fallback_location: hub_url, notice: 'Loan item was NOT successfully updated.' + @loan_item.errors.full_messages.join('; ') }
        format.json { render json: @loan_item.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def destroy
    @loan_item.destroy
    respond_to do |format|
      format.html { redirect_back fallback_location: hub_url, notice: 'Loan item was successfully destroyed.' }
