class SerialChronologiesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @serial_chronology.save
        format.html { redirect_back fallback_location: hub_url, notice: 'Serial chronology was successfully created.' }
        format.json { render json: @serial_chronology, status: :created, location: @serial_chronology }
      else
        format.html { redirect_back fallback_location: hub_url, notice: 'Serial chronology was NOT successfully created.' }
  ...
  def update
    respond_to do |format|
      if @serial_chronology.update(serial_chronology_params)
        format.html { redirect_back fallback_location: hub_url, notice: 'Serial chronology was successfully updated.' }
        format.json { head :no_content }
      else
        format.html { redirect_back fallback_location: hub_url, notice: 'Serial chronology was NOT successfully updated.' }
        format.json { render json: @serial_chronology.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_back fallback_location: hub_url, notice: 'Serial chronology was successfully destroyed.' }
      format.json { head :no_content }
    end
  end
