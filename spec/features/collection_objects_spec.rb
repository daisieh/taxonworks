  ...
  context 'signed in as a user, with some records created' do
    ...
    describe 'GET /api/v1/collection_objects/{id}' do
      ...
      it 'Returns a response including URLs to images API endpoint' do
        visit "/api/v1/collection_objects/#{collection_object.id.to_s}?include[]=images&project_id=#{collection_object.project.id.to_s}&token=#{@user.api_access_token}"
        visit JSON.parse(page.body)['result']['images'].first['url'] + "?project_id=#{collection_object.project.id.to_s}&token=#{@user.api_access_token}"
        expect(JSON.parse(page.body)['result']['id']).to eq(collection_object.images.first.id)
      end

      it 'Returns a response including geo_json' do
        visit "/api/v1/collection_objects/#{collection_object.id.to_s}/geo_json?project_id=#{collection_object.project.id.to_s}&token=#{@user.api_access_token}"
      ...
      it 'Returns a response including images' do
        visit "/api/v1/collection_objects/#{collection_object.id.to_s}/images?project_id=#{collection_object.project.id.to_s}&token=#{@user.api_access_token}"
        visit JSON.parse(page.body)['result']['images'].first['url'] + "?project_id=#{collection_object.project.id.to_s}&token=#{@user.api_access_token}"
        expect(JSON.parse(page.body)['result']['id']).to eq(collection_object.images.first.id)
      end
    end
    ...
    describe 'GET /api/v1/collection_objects/by_identifier/{identifier}' do
      ...
      it 'Returns a response including URLs to collection objects API endpoint' do
        visit "/api/v1/collection_objects/by_identifier/ABCD%20123?project_id=#{collection_object.project.id.to_s}&token=#{@user.api_access_token}"
        visit JSON.parse(page.body)['result']['collection_objects'].first['url'] + "?project_id=#{collection_object.project.id.to_s}&token=#{@user.api_access_token}"
